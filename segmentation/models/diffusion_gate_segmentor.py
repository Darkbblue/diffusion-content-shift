...
class WeightAssigner(nn.Module):
    def __init__(self, dim):
        ...
        self.weight_assigner = nn.Sequential(
    ...
    def forward(self, x):
        return self.weight_assigner(x)

@MODELS.register_module()
class DiffusionGateSegmentor(BaseSegmentor):
        ...
        if isinstance(diffusion_feature, dict):
            ...
            for i, (config, layers) in enumerate(zip(diffusion_feature, feature_layers)):
                feature_extractor = FeatureExtractor(
                    ...
                    control=config['control'] if 'control' in config else None,
                    offline_lora=config['offline_lora'] if 'offline_lora' in config else None,
                )
            ...
            for i, dim in enumerate(c_per_level):
                setattr(self, self.layer_conv_name('weight', i), WeightAssigner(dim))
                setattr(self, self.layer_conv_name('amalgemated', i), MultiRes(dim, 2))
    ...
    def extract_feat(self, inputs: Tensor, is_test=False) -> List[Tensor]:
        ...
        if not self.multiple_diffusion:
            ...
            for i in range(len(outs)):
                # outs[i] = torch.stack(outs[i], dim=1)
                assigner = getattr(self, self.layer_conv_name('weight', i))
                weights = torch.stack([assigner(outs[i][j]) for j in range(len(outs[i]))])  # bucket x batch x d
                weights = F.softmax(weights, dim=0)
                weights = weights.unsqueeze(-1).unsqueeze(-1)
                outs[i] = torch.sum(weights * torch.stack(outs[i]), dim=0)
                outs[i] = getattr(
